...
const Details = () => {
    ...
    useEffect(() => {
        likeService.getOfferLikes(offerId)
            .then(likes => {
    ...
    const likeButtonClick = () => {
        ...
        likeService.like(user._id, offerId)
            .then(() => {
                setOffer(state => ({ ...state, likes: [...state.likes, user._id] }));

                addNotification('Successfuly liked a cat)');
            });
    };

    const userButtons = <Button onClick={likeButtonClick} disabled={offer.likes?.includes(user._id)}>Like</Button>;
